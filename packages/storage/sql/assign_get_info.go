...
func (m *AssignGetInfo) GetBalance(db *DbTransaction, keyId int64) (bool, decimal.Decimal, decimal.Decimal, error) {
	...
	err := GetDB(nil).Table(m.TableName()).
		Where("keyid = ? and deleted =? ", keyId, 0).
		Find(&mps).Error
	if err != nil {
	...
	sp.SetTablePrefix(`1`)
	found1, err1 := sp.Get(nil, `assign_rule`)
	if err1 != nil {
